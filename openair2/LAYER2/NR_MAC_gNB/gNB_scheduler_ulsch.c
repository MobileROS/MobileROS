...
int init_ul_slice_policies() {
  const char *config_path = "../../../rrmPolicy_sub_multiUE.json";
...
static int get_ue_target_slice(int rnti)
{
    ...
    int up_target_sub_slice = 1; // 默认值

    if (json_object_object_get_ex(parsed_json, "ueSliceMapping", &ueSliceMapping)) {
        char key[10] = {0};
        sprintf(key, "0x%04x", rnti);
        struct json_object *ueEntry;
        if (json_object_object_get_ex(ueSliceMapping, key, &ueEntry)) {
            struct json_object *upTargetObj;
            if (!json_object_object_get_ex(ueEntry, "up_target_sub_slice", &upTargetObj)) {
                // 没有则添加默认值到该映射中
                upTargetObj = json_object_new_int(1);
                json_object_object_add(ueEntry, "up_target_sub_slice", upTargetObj);
                // 将更新写回文件
                json_object_to_file(config_path, parsed_json);
            }
            up_target_sub_slice = json_object_get_int(upTargetObj);
        }
    }
    json_object_put(parsed_json);
    return up_target_sub_slice;
}
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  while (remainUEs > 0 && n_rb_sched >= min_rb && iterator->UE != NULL) {
    ...
    printf("-----------------------------Fruit Slice Update-----------------------------------------\n");
    printf("min_rb=%d, max_rb=%d\n", min_rb, max_rbSize);
    printf("-----------------------------Fruit Slice Done------------------------------------------\n");
