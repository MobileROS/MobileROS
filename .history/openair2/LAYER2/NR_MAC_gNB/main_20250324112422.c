...
size_t o1_copy_mac_stats_csv(gNB_MAC_INST *gNB, char *output, size_t strlen, bool reset_rsrp)
{
  ...
  const char *filepath = "../../../rrmPolicy_sub.json";
  ...
  UE_iterator(gNB->UE_info.list, UE) {
    ...
    output += snprintf(output,
            ...
            "%ld,%d,%d,%s,%04x,%d,%d,%f,%f, ,%d,%d,%d,%d,%d,%d,%u,%u,%d,%d,%"PRIu32",%"PRIu16", ,%.5f,%d,%"PRIu64",%"PRIu64",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu64",%d,%d,%02d%02d%02d,%"PRIu64",%d,%d,%02d%02d%02d, ,%.5f,%d,%"PRIu64",%"PRIu64",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32",%d,%s, ,\n",
            timestamp,
            gNB->frame,
            gNB->slot,
            ue_imsi,
    ...
    snprintf(influx_data, sizeof(influx_data),
          "glasses_18,rnti=%04x in_sync=%d,frame=%d,slot=%d,DL_Thr=%f,UL_Thr=%f,"
