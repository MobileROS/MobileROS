...
class RMMPolicyController:
    ...
    def update_rmmpolicy(self):
        ...
        if len(ue_keys) >= self.last_n_ues:
            for ue_key in ue_keys[-self.last_n_ues:]:
                target_sub = random.randint(1, len(data["subSlicePolicy"]["subSlices"]))
                up_target_sub = random.randint(1, len(data["subSlicePolicy"]["subSlices"]))
                
                # 更新UE映射
                data["ueSliceMapping"][ue_key].update({
                    "target_sub_slice": target_sub,
                    "up_target_sub_slice": up_target_sub
                })
                
                # 更新sliceConfigId: 前两位(12)保持不变，后两位使用target_sub_slice和up_target_sub_slice
                new_config_id = 1200 + target_sub * 10 + up_target_sub
                data["sliceConfigId"] = new_config_id
