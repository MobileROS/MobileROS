...
class RMMPolicyController:
    ...
    def update_rmmpolicy(self):
        ...
        # # 处理up_rrmPolicyRatio（保持原有逻辑）
        # current_slices = len(data["up_rrmPolicyRatio"])
        # if current_slices < self.max_slices:
        #     new_slice = self.generate_new_slice()
        #     data["up_rrmPolicyRatio"].append(new_slice)

        # # 只更新subSlicePolicy中的min_ratio和max_ratio
        # for sub_slice in data["subSlicePolicy"]["subSlices"]:
        #     sub_slice["min_ratio"] = random.randint(0, 40)
        #     sub_slice["max_ratio"] = random.randint(60, 100)

        # 获取Slice_Config的后两位数字
        slice_config = str(data.get("Slice_Config", "1234"))  # 默认值1234
        if len(slice_config) >= 4:
            target_sub_slice = int(slice_config[-2])  # 倒数第二位
            up_target_sub_slice = int(slice_config[-1])  # 最后一位
        else:
            target_sub_slice = 1  # 默认值
            up_target_sub_slice = 1  # 默认值

        # 更新最后N个UE的映射，如果up_target_sub_slice不存在则补充
        ...
        if len(ue_keys) >= self.last_n_ues:
            for ue_key in ue_keys[-self.last_n_ues:]:
                ...
                mapping["target_sub_slice"] = target_sub_slice
                if "up_target_sub_slice" not in mapping:
                    mapping["up_target_sub_slice"] = up_target_sub_slice
